...
//verify https://atcoder.jp/contests/abc215/tasks/abc215_f
...
int main(){
    ...
    OrthogonalRangeCount<ll> orc(P);
    ll left=0,right=1000000003;
    while(right-left>1){
        ll mid=(left+right)/2;
        bool ok=false;
        for(int i=0;i<N;i++){
            ll x=P[i].fi,y=P[i].se;
            if(orc.query(x+mid,y+mid,INF<ll>,INF<ll>)) ok=true;
            if(orc.query(x+mid,-INF<ll>,INF<ll>,y-mid+1)) ok=true;
            if(orc.query(-INF<ll>,y+mid,x-mid+1,INF<ll>)) ok=true;
            if(orc.query(-INF<ll>,-INF<ll>,x-mid+1,y-mid+1)) ok=true;
            if(ok) break;
        }
        if(ok) left=mid;
        else right=mid;
